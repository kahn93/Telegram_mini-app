...
const ArcadeAsteroids: React.FC<{ onBack: () => void; playerName: string }> = ({ onBack, playerName }) => {
  const [submitError, setSubmitError] = React.useState('');
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
      <button onClick={onBack} style={{ alignSelf: 'flex-start', margin: 8 }}>← Back to Arcade</button>
      <Asteroids onScore={async score => {
        if (score > 0) {
          try {
            const mod = await import('../ArcadeGames/leaderboardSupabase');
            await mod.submitScoreSupabase('Asteroids', playerName, score);
            setSubmitError('');
          } catch (e: unknown) {
